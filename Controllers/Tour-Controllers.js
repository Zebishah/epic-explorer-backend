...
export const addTour = async (req, res, next) => {
  ...
  if (existingTour) {
    ...
    return res
      .status(400)
      .json({ success, message: "Tour already existed ", statusCode: 400 });
  }
  let tour;
  try {
    let gallery = [],
  ...
  const parentCategoryId = new mongoose.Types.ObjectId(parentCategory);
  let category;
  try {
    category = await Categorie.findOne({ _id: parentCategoryId });
    console.log(category);
    category.items.push(new mongoose.Types.ObjectId(tour.id));
    category.save();
  } catch (error) {
    return next(error);
  }
  if (!category) {
    return res
      .status(400)
      .json({ success: false, message: "category not found", statusCode: 400 });
  }
  let date = new Date();

  let notificationAdmin = new NotificationsAdmin({
    accommodationName: "Tour Added Successfully",
    Category: "Tour Added",
    message: `One Tour ${name} is added to our site`,
    date,
  });
...
export const deleteTour = async (req, res, next) => {
  ...
  try {
    ...
    if (!tour) {
      return res
        .status(400)
        .json({ success: false, message: "Tour not found for deletion" });
    }

    let date = new Date();

    let notificationAdmin = new NotificationsAdmin({
      accommodationName: "Tour deleted Successfully",
      Category: "Tour deleted",
      message: `One Tour ${tour.name} is deleted from our site`,
      date,
    });
    await notificationAdmin.save();
    ...
    let notificationUser = new NotificationsUser({
      ...
      accommodationName: "Tour deleted Successfully",
      Category: "Tour deleted",
      message: `One Tour ${tour.name} is deleted to our site`,
    });
    await notificationUser.save();
...
export const updateTour = async (req, res, next) => {
  ...
  tour.Departure_ReturnLocation =
    Departure_ReturnLocation || tour.Departure_ReturnLocation;
  ...
  let notificationAdmin = new NotificationsAdmin({
    accommodationName: "Tour updated Successfully",
    Category: "Tour updated",
    message: `One Tour ${tour.name} is updated in our site`,
    date,
  });
  await notificationAdmin.save();
  ...
  return res
    .status(200)
    .json({ success: true, message: "Tour updated successfully", tour: tour });
};
export const filterTours = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error fetching tours", error });
  }

  if (!filteredTours || filteredTours.length === 0) {
    return res.status(404).json({
      success: false,
      message: "No tours found with the given filter",
    });
  }
  return res.status(200).json({ success: true, tours: filteredTours });
};
export const customizedToursSearch = async (req, res, next) => {
  let { tourLocation, membersLimit, days } = req.body;

  const queryObj = {};

  // Handle the membersLimit filter
  if (membersLimit) {
    queryObj.membersLimit = Number(membersLimit);
  }

  // Handle the tourLocation filter
  if (tourLocation) {
    queryObj.tourLocation = tourLocation;
  }

  // Handle the tourLocation filter
  if (days) {
    queryObj.days = Number(days);
  }
  console.log(queryObj);
...
export const countTours = async (req, res, next) => {
  ...
  res.status(200).json({
    success,
    message: "here is your tour count",
...
export const openTour = async (req, res, next) => {
  ...
  if (!tour) {
    ...
    return res
      .status(400)
      .json({ success, message: "Tour not existed that u are trying to open" });
  }
  ...
  if (!tourServiceIt) {
    ...
    return res.status(400).json({
      success,
      message: "Tour services not existed that u are trying to open",
    });
  }

  return res
    .status(200)
    .json({ success: true, tour: tour, tourServiceIt: tourServiceIt });
};
...
export const getFormData = async (req, res, next) => {
  ...
  try {
    tourBookingCheck = await BookingTour.findOne({
      tourId: id,
  ...
  if (tourBookingCheck) {
    return res
      .status(400)
      .json({ success, message: "Tour already existed in booking database" });
  }
  let tourBooking;
  tour.price = tour.price * members;
  try {
    // tourNo = tourNo + 1;
    tourBooking = new BookingTour({
      tourId: tour.id,
      checkInDate,
      dropOffLocation,
      name: tour.name,
...
export const searchTourByName = async (req, res, next) => {
  ...
  if (tours == "") {
    return res
      .status(400)
      .json({ success: false, message: "no tours found in database" });
  }

  const filteredTours = tours.filter((
    tour //filtering searched tour information
  ) => tour.name.toLowerCase().includes(name.toLowerCase()));

  return res.status(200).json({
    success: true,
    message: "here are your all tours",
...
export const tourPackages = async (req, res, next) => {
  ...
  console.log(familyTours);

  return res.status(200).json({
    success: true,
    message: "here are your all tours",
    honeymoonTours: honeymoonTours,
    familyTours: familyTours,
...
export const extraPaymentDetail = async (req, res, next) => {
  let id = req.body.id;
  console.log(id);
  let tours;
  try {
    tours = await BookingTour.findOne({ tourId: id });
  } catch (error) {
    return next(error);
  }
  ...
  let admin = process.env.ADMIN_ACCOUNT_ID;
  return res.status(200).json({
    success: true,
    message: "here are your all tours",
    tours: tours,
...
export const extraPaymentDetailTransport = async (req, res, next) => {
  let id = req.body.id;
  console.log(id);
  let transport;
  try {
    transport = await BookingTransport.findOne({ transportId: id });
  } catch (error) {
    return next(error);
  }
...
export const extraHPaymentDetailTransport = async (req, res, next) => {
  let id = req.body.id;
  console.log(id);
  let room;
  try {
    room = await BookingHotel.findOne({ roomId: id });
  } catch (error) {
    return next(error);
  }
  ...
  if (!room) {
    return res
      .status(400)
      .json({ success: false, message: "no room found in database" });
  }

  let admin = process.env.ADMIN_ACCOUNT_ID;
  return res.status(200).json({
    success: true,
    message: "here are your all room",
    room: room,
    admin: admin,
  });
};
